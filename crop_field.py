#!/usr/bin/env python3
"""
crop_field.py — обрезка изображений по заранее заданным размерам.

Принимает на вход пути к изображениям и изменяет их же (in-place).

Логика:
    CROP_FIELD — словарь вида {(width, height): (x1, y1, x2, y2)}.
    Для каждого входного изображения берём его реальные (width, height),
    ищем подходящий ключ в CROP_FIELD и обрезаем по указанному прямоугольнику.
    Если ключа нет — изображение пропускается.

Пример:
    python crop_field.py img1.png img2.png img3.png
    python crop_field.py ./images/*.png
"""
...
from constants import CROP_FIELD
...
def crop_image(path: Path) -> bool:
    ...
    h, w = img.shape[:2]
    key = (w, h)
    if key not in CROP_FIELD:
        print(f"[=] Пропуск (нет правила для {w}x{h}): {path}")
        return False

    x1, y1, x2, y2 = CROP_FIELD[key]
...
def main():
    if len(sys.argv) < 2:
        print("Использование: python crop_field.py <image1> [image2 ...]")
        sys.exit(1)
